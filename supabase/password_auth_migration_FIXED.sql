-- ==========================================================================
-- Asokamanpower — Password-Based Authentication System Migration (FIXED)
...
DROP POLICY IF EXISTS "public can create accounts" ON user_accounts;
...
DROP POLICY IF EXISTS "public can read own account" ON user_accounts;
...
DROP POLICY IF EXISTS "public can update own account" ON user_accounts;
...
DROP POLICY IF EXISTS "admin all user_accounts" ON user_accounts;
...
DROP POLICY IF EXISTS "users can read own candidate submissions" ON candidates;
...
DROP POLICY IF EXISTS "users can read own employer submissions" ON employers;
...
-- Function to generate temporary password (8 characters: TEMPXXXX)
...
BEGIN
  ...
  WHERE ua.email = p_email
    AND ua.password_hash = hash_password(p_password);
END;
...
GRANT EXECUTE ON FUNCTION get_or_create_user_account(TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION get_user_submissions(UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION change_user_password(UUID, TEXT, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION update_theme_preference(UUID, TEXT) TO anon, authenticated;
...
BEGIN
  -- Migrate candidates
  FOR r IN (SELECT DISTINCT email FROM candidates WHERE email IS NOT NULL AND email != '' AND email NOT IN (SELECT email FROM user_accounts))
  LOOP
    -- Create user account
    ...
    RAISE NOTICE 'Created account for %: Temporary password = %', r.email, v_temp_pwd;
  END LOOP;

  -- Migrate employers
  FOR r IN (SELECT DISTINCT email FROM employers WHERE email IS NOT NULL AND email != '' AND email NOT IN (SELECT email FROM user_accounts))
  LOOP
    v_temp_pwd := generate_temp_password();
    INSERT INTO user_accounts (email, password_hash, is_temp_password)
    VALUES (r.email, hash_password(v_temp_pwd), true)
  ...
  -- Update existing users' submissions that don't have user_id yet
  UPDATE candidates c
  SET user_id = (SELECT id FROM user_accounts WHERE email = c.email)
  WHERE c.user_id IS NULL AND c.email IN (SELECT email FROM user_accounts);

  UPDATE employers e
  SET user_id = (SELECT id FROM user_accounts WHERE email = e.email)
  WHERE e.user_id IS NULL AND e.email IN (SELECT email FROM user_accounts);
...
DECLARE
  ...
  v_policy_count INT;
BEGIN
  SELECT COUNT(*) INTO v_table_count FROM information_schema.tables WHERE table_name = 'user_accounts';
  SELECT COUNT(*) INTO v_function_count FROM pg_proc WHERE proname IN ('verify_password', 'get_or_create_user_account', 'get_user_submissions');
  SELECT COUNT(*) INTO v_policy_count FROM pg_policies WHERE tablename = 'user_accounts';
  ...
  RAISE NOTICE 'You can now use the password-based authentication system!';
  RAISE NOTICE '========================================';
END $$;
...
-- 2. All functions are working (7 functions)
-- 3. RLS policies are active
-- 4. Existing data is migrated (if any)
-- 5. Temporary passwords are shown in NOTICES above
-- 6. New form submissions will automatically create user accounts
-- 7. Users can login with email + password
-- 8. Users can see all their submissions in one dashboard
-- 9. Theme preference is saved per user

-- Done! Your password-based authentication system is ready to use.
